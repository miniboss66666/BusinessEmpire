...
const Auth = (() => {
    ...
    if (!email) {
      UI.showModal(`
        <div style="text-align:center">
          ...
          <p style="color:var(--text-dim);margin-bottom:16px">Nhập email của bạn vào ô đăng nhập trước</p>
          <button class="btn-primary" onclick="UI.closeModal()">OK</button>
        </div>
      `);
      return;
  ...
  // ── Fix: dùng explicit fields thay vì select('*') để tránh 406 ──
  async function loadProfile() {
    const { data, error } = await DB
      .from('profiles')
      .select('id, username, role, avatar_url, created_at')
      .eq('id', STATE.user.id)
      .maybeSingle();  // maybeSingle() thay vì single() — không throw 406 khi không có row

    if (error) {
      console.warn('[Auth] loadProfile error:', error.message);
      STATE.profile = { id: STATE.user.id, username: null, role: 'user', avatar_url: null };
      return;
    }
    ...
    if (!data) {
      ...
      const { data: newProfile } = await DB
        .from('profiles')
        .insert({ id: STATE.user.id, username: null, role: 'user' })
        .select('id, username, role, avatar_url, created_at')
        .single();
      STATE.profile = newProfile || { id: STATE.user.id, username: null, role: 'user', avatar_url: null };
      return;
    }
  ...
  return { checkSession, logout, loginWithOAuth };
})();